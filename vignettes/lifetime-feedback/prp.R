...
prp_base <- function(path.prp, prefix) {
    df.prp.sw <- ldply(c("phase", "cwp", "cf", "cloud"), function(pert) {
    ...
    df.prp.lw <- ldply(c("phase", "cwp", "cf", "cloud"), function(pert) {
...
prp.global <- function(path.prp) {
    df.prp.sw <- ldply(c("phase", "cwp"), function(pert) {
    ...
    df.prp.lw <- ldply(c("phase", "cwp"), function(pert) {
...
expname.prp <- function(ccraut, ccauloc, creth,
    ...
    experiment <- sprintf("~/echam-phase-prp/rain_%g%s%s%s%s%s%s%s%s",
                          ccraut,
    ...
    plyr::ddply(expand.grid(ccraut = ccraut,
                ...
                function(x) with(x, {
                    ...
                    if (!is.na(file.info(experiment.nocosp)$isdir))
                        df <- prp(experiment.nocosp)
                    else
                        df <- prp(experiment.cosp)
                    df %>% 
